...
def batch_crop_pdbs(pdb_id_list, dir_experimental, pdb_to_seq_id, seq_lookup):
    """
    Iterates through a list of PDB IDs.
    Finds their sequence. Crops them.
    Returns a NEW list of IDs pointing to the cropped files.
    """
    print(f"    [Optimization] Pre-cropping {len(pdb_id_list)} structures to speed up calculation...")
    new_id_list = []
    
    for pdb_id in pdb_id_list:
        original_path = get_actual_structure_path(dir_experimental, pdb_id)
        if not original_path:
            continue
            
        # Determine Target Sequence
        target_seq = None
        if pdb_id.lower() in pdb_to_seq_id:
            seq_id = pdb_to_seq_id[pdb_id.lower()]
            if seq_id in seq_lookup:
                target_seq = seq_lookup[seq_id]
        
        if target_seq:
            # Define new cropped filename
            # We use "_cropped" suffix so we don't overwrite the original (safety)
            # But we return this new ID so the matrix calculator uses it
            cropped_id = f"{pdb_id}_cropped"
            cropped_path = os.path.join(dir_experimental, f"{cropped_id}.pdb")
            
            # Run Cropping (only if not already done)
            if not os.path.exists(cropped_path):
                result_path = align_and_crop_single_pdb(original_path, target_seq, cropped_path)
                
                # If cropping failed (returned original), use original ID
                if result_path == original_path:
                    new_id_list.append(pdb_id)
                else:
                    new_id_list.append(cropped_id)
            else:
                new_id_list.append(cropped_id)
        else:
            # No sequence map found, keep original
            new_id_list.append(pdb_id)
...
def get_aligned_matrices(cov_path, tm_df, sample_list, sort_by="groups"):
    ...
    if sort_by == "groups":
        for sample_id in sample_list:
            ...
            if norm_sample in cov_map and norm_sample in tm_map: target_order_norm.append(norm_sample)
        ...
        for cov_idx in df_cov_full.index:
            ...
            if norm_cov in sample_set_norm and norm_cov in tm_map: target_order_norm.append(norm_cov)
    if not target_order_norm: return None, None
...
def visualize_structures_pipeline(fasta_path, split_data, sig_split_folder, ordered_cov_path):
    """ Main Pipeline. """
    # Get the local subfamily directory and its parent (the main calc_dir)
    local_dir = os.path.dirname(fasta_path)
    parent_dir = os.path.dirname(local_dir)
    
    # Define potential paths
    global_structures = os.path.join(parent_dir, 'structures')
    local_structures = os.path.join(local_dir, 'structures')
    ...
    if df_sorted is not None and not df_sorted.empty:
        ...
        if rep_a_id and rep_b_id:
            ...
            if os.path.exists(pdb_a) and os.path.exists(pdb_b):
                align_and_visualize_pair(pdb_a, pdb_b, align_output, label_a=f"Group A (Rep: {rep_a_id})", label_b=f"Group B (Rep: {rep_b_id})")
        ...
        if cov_ord is not None:
            plot_side_by_side_dynamic(cov_ord, tm_ord, sample_a, sample_b, os.path.join(sig_split_folder, "combined_ordered_by_covariance.png"), "(Ordered by Covariance Index)")
    ...
    if global_map:
        for seq_id, pdbs in global_map.items():
            for pdb in pdbs: pdb_to_seq_id[pdb.lower()] = seq_id

    if global_map:
        augmented_map = global_map.copy()
        for key in list(global_map.keys()):
            norm_key = normalize_id(key)
            if norm_key not in augmented_map: augmented_map[norm_key] = global_map[key]
        
        success, exp_a_ids, exp_b_ids = check_and_download_structures(augmented_map, split_data['group_a'], split_data['group_b'], dir_experimental)
        ...
        if success:
            ...
            if not has_enough_for_viz:
                ...
                # --- NEW STEP: BATCH CROP EVERYTHING BEFORE CALCULATION ---
                # This ensures TM-align runs on tiny files, not giant complexes
                clean_a_cropped = batch_crop_pdbs(clean_a, dir_experimental, pdb_to_seq_id, seq_lookup)
                clean_b_cropped = batch_crop_pdbs(clean_b, dir_experimental, pdb_to_seq_id, seq_lookup)
                
                print(f"Calculating TM Matrix (Experimental) for {len(clean_a_cropped) + len(clean_b_cropped)} structures...")
                
                # Pass cropped IDs to calculation
                df_exp, stats_exp, split_exp = calculate_tm_matrix(clean_a_cropped, clean_b_cropped, dir_experimental)
                
                if df_exp is not None and not df_exp.empty:
                    if has_enough_for_tm:
                        plot_experimental_grouped_tm(df_exp, clean_a_cropped, clean_b_cropped, os.path.join(sig_split_folder, "experimental_tm_ordered_by_groups.png"))

                    print("\n=== Generating Representative Alignment (Experimental) ===")
                    rep_a_exp = get_group_representative(df_exp, clean_a_cropped)
                    rep_b_exp = get_group_representative(df_exp, clean_b_cropped)
